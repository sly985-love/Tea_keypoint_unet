    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            with open(json_file, 'r') as f:
                ...
                #     # print(all_P)
                # print(all_P)
                # if ("leaftop"in all_P)and("budtop"in all_P) and("leafbottom"in all_P)and("budbottom"in all_P)and("leafmedium"in all_P)and("stembottom"in all_P) :
                #     pass
                # else:
                #     print("{} 有问题！".format(json_file))
                ...
                # 将所有关键点draw_umich_gaussian画成一张热力图，再将六张热力图融合拼接成一张热力图
            heatmap_0 = np.zeros((self.img_h, self.img_w))
            draw_umich_gaussian(heatmap_0, (gt[0][0] * (self.resize_W / W), gt[0][1] * (self.resize_H / H)), 30)
            heatmap_1 = np.zeros((self.img_h, self.img_w))
            draw_umich_gaussian(heatmap_1, (gt[1][0] * (self.resize_W / W), gt[1][1] * (self.resize_H / H)), 30)
