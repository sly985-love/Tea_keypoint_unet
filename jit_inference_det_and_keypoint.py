...
def keypoint_det(cv2img, img, model, p1, transforms_kp):
    ...
    cv2.line(cv2img, (int(point_0_new[0]), int(point_0_new[1])), (int(point_1_new[0]), int(point_1_new[1])), (0, 0, 255), 1,8)
    # cv2.line(cv2img, (point_1_new[0], point_1_new[1]), (point_2_new[0], point_2_new[1]), 8,s (255, 0, 255), -1)
    # cv2.line(cv2img, (point_2_new[0], point_2_new[1]), (point_3_new[0], point_3_new[1]), 8, (255, 0, 255), -1)
    # cv2.line(cv2img, (point_3_new[0], point_3_new[1]), (point_4_new[0], point_4_new[1]), 8, (255, 0, 255), -1)
    # cv2.line(cv2img, (point_4_new[0], point_4_new[1]), (point_5_new[0], point_5_new[1]), 8, (255, 0, 255), -1)
    # cv2.line(cv2img, (point_5_new[0], point_5_new[1]), (point_4_new[0], point_4_new[1]), 8, (255, 0, 255), -1)
...
if __name__ == '__main__':
    ...
    cv2img = cv2.imread(r"data/src_picture/IMG_5795.JPG")
    h, w = cv2img.shape[:2]
    resize_ = [640, 640]
    ...
    torchScript_output = non_max_suppression(torch_script_model_inference, conf_thres=0.25, iou_thres=0.45,
                                             agnostic=False)

    for i, det in enumerate(torchScript_output):  # detections per image
        for d in range(det.shape[0]):
            x1, y1, x2, y2 = det[d, 0].cpu().numpy() * (w / resize_[1]), det[d, 1].cpu().numpy() * (h / resize_[0]), \
                             det[d, 2].cpu().numpy() * (w / resize_[1]), det[d, 3].cpu().numpy() * (h / resize_[0])
    ...
    cv2img = cv2.resize(cv2img, (1366, 768))
    # cv2img = cv2.resize(cv2img, (1920, 1080))
    # cv2img = cv2.resize(cv2img, (2560, 960))
